...
export function activityToFit(normalized) {
  ...
  if (records.length > 0) {
    writeDef(w, LOCAL.RECORD, MESG.RECORD, [
      ...
      { defNum: 2,   type: BT.UINT16 }  // altitude (m, scale 5, offset 500)
    ]);
    for (const rec of records) {
      ...
      // Altitude FIT formula: (alt_m + 500) * 5
      const altRaw = Number.isFinite(rec.altitude)
        ? Math.round((rec.altitude + 500) * 5) : null;
  ...
  if (laps.length > 0) {
    writeDef(w, LOCAL.LAP, MESG.LAP, [
      ...
      { defNum: 22,  type: BT.UINT16 }, // total_ascent
      { defNum: 23,  type: BT.UINT16 }  // total_descent
    ]);
    for (const lap of laps) {
      ...
      const lapAscent = Number.isFinite(lap.totalAscent) ? lap.totalAscent : 
                        (normalized.totalAscent ? normalized.totalAscent * (lap.distanceMeters / (normalized.totalDistanceMeters || 1)) : 0);
      ...
      writeData(w, LOCAL.LAP, [
        ...
        { type: BT.UINT16, value: lapAscent },
        { type: BT.UINT16, value: 0 }
      ]);
    }
  }
  ...
  writeData(w, LOCAL.SESSION, [
    ...
    { type: BT.UINT16, value: normalized.totalAscent ?? 0 },
    { type: BT.UINT16, value: normalized.totalDescent ?? 0 }
  ]);
